...
let DYNAMIC_PENGADUAN_MEMORY: PengaduanWarga[] = [];
...
export async function getMallilingiDataAsync(): Promise<MallilingiData> {
  if (supabaseClient) {
    try {
      ...
      const dbPengaduan = pengaduanRes.data && pengaduanRes.data.length > 0 ? pengaduanRes.data : DEFAULT_MALLILINGI_DATA.pengaduan;
      const combinedPengaduan = [...DYNAMIC_PENGADUAN_MEMORY, ...dbPengaduan].filter(
  ...
  return {
    ...
    pengaduan: [...DYNAMIC_PENGADUAN_MEMORY, ...DEFAULT_MALLILINGI_DATA.pengaduan],
...
export async function sendPengaduanWargaAsync(aduan: { nama: string; telepon: string; kategori: string; judul: string; isi: string }): Promise<boolean> {
  ...
  DYNAMIC_PENGADUAN_MEMORY = [newAduan, ...DYNAMIC_PENGADUAN_MEMORY];
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      const existing: PengaduanWarga[] = stored ? JSON.parse(stored) : [];
      localStorage.setItem("mallilingi_pengaduan_list", JSON.stringify([newAduan, ...existing]));
    } catch (e) {
